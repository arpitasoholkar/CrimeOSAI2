...
//   const [pageStart, setPageStart] = useState(Math.max(0, timeline.length - PAGE_SIZE))

//   useEffect(() => {
//     setPageStart(Math.max(0, timeline.length - PAGE_SIZE))
//   }, [timeline.length])
...
//   const atEnd = pageStart + PAGE_SIZE >= timeline.length
//   const visible = timeline.slice(pageStart, pageStart + PAGE_SIZE)

//   return (
//     <section className={styles.section}>
//       <div className={styles.timelineHeadRow}>
//         <div className={styles.timelineHeadIcon}>
//           <HistoryIcon width={16} height={16} />
//         </div>
//         <div style={{ flex: 1 }}>
//           <h3 className={styles.sectionTitle} style={{ margin: 0 }}>Investigation Timeline</h3>
//           <p className={styles.metaText}>Latest {Math.min(PAGE_SIZE, timeline.length)} updates from your investigation</p>
//         </div>
...
//             className={styles.timelineNavBtn}
//             disabled={atEnd}
//             onClick={() => setPageStart((p) => Math.min(timeline.length - 1, p + PAGE_SIZE))}
...
//           <button
//             type="button"
//             className={styles.timelineLatestBtn}
//             disabled={atEnd}
//             onClick={() => setPageStart(Math.max(0, timeline.length - PAGE_SIZE))}
//           >
//             <span className={styles.timelineLatestDot} /> Latest
//           </button>
//         </div>
//       </div>
...
//             const isAi = AI_ACTIONS.has(t.action)

//             return (
//               <div key={t.sequence} className={styles.timelineStep}>
//                 {isLast ? (
//   <span className={styles.timelineNodeCurrent}>
//     <span className={styles.timelineNodeCurrentInner} />
//   </span>
// ) : (
//   <span className={styles.timelineNodeDone} />
// )}
//                 {isLast && <span className={styles.timelineLatestTag}>LATEST</span>}
//                 <span className={`${styles.timelineStepLabel} ${isLast ? styles.timelineStepLabelCurrent : ''}`}>
//                   {isAi && <SparklesIcon width={11} height={11} />}
//                   {toTitleCase(t.action)}
//                 </span>
//                 <span className={styles.timelineStepTime}>{fmtDate(t.timestamp)}</span>
//               </div>
//             )
//           })}
//         </div>
...
//         Showing latest {visible.length} updates. New updates appear on the right.
//       </p>
//     </section>
//   )
// }
...
function TimelinePanel({ timeline }) {
  ...
  // Jump straight to the latest event on first load, then smooth-scroll
  // to reveal new ones as they arrive (instead of snapping instantly).
  useEffect(() => {
  ...
  // Let a plain vertical mouse-wheel / trackpad scroll drive the horizontal track.
  useEffect(() => {
    const el = trackWrapRef.current
    if (!el) return
    const onWheel = (e) => {
  ...
  const onKeyDown = (e) => {
    const el = trackWrapRef.current
    if (!el) return
    if (e.key === 'ArrowRight') { el.scrollBy({ left: 140, behavior: 'smooth' }); e.preventDefault() }
    if (e.key === 'ArrowLeft') { el.scrollBy({ left: -140, behavior: 'smooth' }); e.preventDefault() }
  }
  ...
  const onPointerMove = (e) => {
    ...
    trackWrapRef.current.scrollLeft = drag.current.startScroll - (e.clientX - drag.current.startX)
  }
  const endDrag = () => {
    if (!drag.current.active) return
    drag.current.active = false
    trackWrapRef.current?.classList.remove(styles.dragging)
  }
  ...
  return (
    <section className={styles.section}>
      <div className={styles.timelineHeadRow}>
        ...
        <div style={{ flex: 1 }}>
          <h3 className={styles.sectionTitle} style={{ margin: 0 }}>Investigation Timeline</h3>
          <p className={styles.metaText}>{timeline.length} update{timeline.length !== 1 ? 's' : ''} · scroll or drag to explore</p>
        </div>
      </div>
          ...
          {timeline.map((t, i) => {
            ...
            return (
              <div key={t.sequence} className={styles.timelineStep}>
                ...
                {isLast && <span className={styles.timelineLatestTag}>LATEST</span>}
                <span className={`${styles.timelineStepLabel} ${isLast ? styles.timelineStepLabelCurrent : ''}`}>
                  {toTitleCase(t.action).replace(/^Ai /, '')}
                </span>
                <span className={styles.timelineStepTime}>{fmtTimelineTime(t.timestamp)}</span>
              </div>
            )
          })}
        </div>
      ...
      <p className={styles.timelineFooterNote}>
        Showing all {timeline.length} updates. Scroll, drag, or use the arrow keys to browse.
      </p>
    </section>
  )
}
