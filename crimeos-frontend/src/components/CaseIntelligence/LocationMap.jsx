...
//  * Renders geocoded case entities as pins on a real, interactive map.
//  *
//  * Google Maps is the default renderer. It requires VITE_GOOGLE_MAPS_API_KEY
//  * to be set (see .env.example) -- if it's missing, we show a clear setup
//  * message rather than silently swapping in a different map provider, since
//  * a silent substitution is easy to miss and harder to debug later.
//  *
//  * Leaflet + CARTO basemap tiles are still available as an explicit opt-in
//  * (no API key/billing needed) via <LocationMap provider="leaflet" />, for
//  * local dev or environments that don't want to configure Google Maps.
//  *
//  * @param {{ id: string, label: string, lat: number, lng: number, type?: string, detail?: string }[]} markers
//  * @param {'google' | 'leaflet'} [provider] - which map renderer to use. Defaults to 'google'.
//  */
// export default function LocationMap({ markers = [], provider = 'google' }) {
//   if (provider === 'leaflet') return <LeafletMapView markers={markers} />

//   if (!GOOGLE_MAPS_KEY) {
//     return (
//       <p className={styles.error}>
//         Google Maps isn't configured yet. Set <code>VITE_GOOGLE_MAPS_API_KEY</code> in your
//         .env file to enable it, or pass <code>provider="leaflet"</code> to use the free
//         CARTO/OpenStreetMap map instead.
//       </p>
//     )
//   }
...
// // ---------------------------------------------------------------------
// // Leaflet / CARTO basemap tiles (opt-in, no API key needed)
// // ---------------------------------------------------------------------
...
// // ---------------------------------------------------------------------
// // Google Maps JavaScript API (default renderer)
// // ---------------------------------------------------------------------
  ...
  useEffect(() => {
    ...
    import('leaflet')
        ...
        if (markers.length) {
          ...
          for (const m of markers) {
            ...
            L.marker([m.lat, m.lng], { icon })
              .addTo(layerRef.current)
              .bindPopup(
                `<strong>${escapeHtml(m.label)}</strong><br/>${escapeHtml(
                  m.detail || ''
                )}`
              )
      ...
      .catch((err) => {
        ...
        if (!cancelled) {
          setError(
            'Map library failed to load. Run "npm install leaflet" in crimeos-frontend.'
          )
        }
      })
